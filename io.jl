...
function parseparams(rawdata :: Array{String})
    ...
    qa = Dict(el_ord_par .=> parseline(Float64,rawdata[9]))
    ...
    for i in 1:ntypes
        refcn[i] = parseline(Float64,rawdata[12+i])
    end
    ...
    for _ in 1:ntypes*(ntypes+1)/2
        index = Tuple(parseline(Int64,rawdata[linenum+1])[1:2])
        c_ref[index] = parsematrix(numrefcn[index[1]],numrefcn[index[2]],rawdata[linenum+2:linenum+2+numrefcn[index[1]]])
    ...
    linenum += 1
    k_en = parseline(Float64,rawdata[linenum])
    linenum += 1
    covradii = parseline(Float64,rawdata[linenum])
    r_cov = covradii ./ BORH_TO_Å
    linenum += 1
    γ = parseline(Float64,rawdata[linenum])
    ...
    return BORH_TO_Å, EV_TO_AU, k_f, α, z_eff, a1, a2, s6, s8, k_cn, k_l, sc_radii, refcn, c_ref,
    nprim, ζ, d, k_ab, nk_ll, k_ll, electroneg, k_en, r_cov, γ, std_sh_pop, η_a, h_al, k_poli, k_cn_l
end
